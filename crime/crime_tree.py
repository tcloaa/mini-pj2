...
print "Mean and std of the cross-validation"
print scores.mean()
print scores.std()
pred = regr.predict(X_test)
print "MSE just use regression tree"
print mean_squared_error(y_test, pred)

print X.shape

# bagging using all features
regr1 = RandomForestRegressor(max_features=23)
regr1.fit(X_train, y_train.ravel())
pred1 = regr1.predict(X_test)
print "MSE-random-forest using all features"
print mean_squared_error(y_test, pred1)

# less features
regr2 = RandomForestRegressor(max_features=10)
regr2.fit(X_train, y_train.ravel())
pred2 = regr2.predict(X_test)
# print mean_squared_error(y_test, pred2)

# boosting
regr3 = GradientBoostingRegressor(n_estimators=500, learning_rate=0.01)
...
Importance = pd.DataFrame({'Importance':regr3.feature_importances_*100}, index=['rincpc','econgrow','unemp','citypop','a0_5','a5_9','a10_14','a15_19','a20_24','a25_29','citybla','cityfemh','sta_educ','sta_welf','price','sworn','civil','elecyear','governor','term2','term3','termlim','mayor'])

Importance.sort_values('Importance', axis=0, ascending=True).plot(kind='barh', color='r', )
plt.xlabel('Variable Importance')
plt.gca().legend_ = None
plt.savefig("importance_2.png")
